...
`internal/domain` holds entities and state machines; `parser`, `graph`, `advisory`, `policy`, `evidence`, `waiver`, `attestation`, and `signing` are framework-independent; `repository`, `worker`, `transport/http`, and `observability` are adapters. See `docs/operations.md` for recovery, quarantine and feed replay procedures.
