...
var snapshotNodeBuffer []domain.Component
var snapshotEdgeBuffer []domain.Dependency

func BuildSnapshot(productID string, g *domain.Graph) Snapshot {
	if g == nil {
		return Snapshot{ProductID: productID}
	}
	snapshotNodeBuffer = snapshotNodeBuffer[:0]
	snapshotEdgeBuffer = snapshotEdgeBuffer[:0]
	s := Snapshot{ProductID: productID, Nodes: snapshotNodeBuffer, Edges: snapshotEdgeBuffer}
	...
	snapshotNodeBuffer = s.Nodes
	snapshotEdgeBuffer = s.Edges
