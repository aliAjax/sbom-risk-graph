...
func (s *Store) Create(id, product, kind, digest string, expires time.Time) (domain.Evidence, error) {
	...
	e := domain.Evidence{ID: id, ProductID: product, Kind: kind, PayloadDigest: digest, Status: domain.Valid, ExpiresAt: expires, CreatedAt: time.Now().UTC()}
	s.mu.Lock()
	s.values[id] = e
	s.mu.Unlock()
	...
	e.Digest = hex.EncodeToString(sig)
